...
const EWMA_ALPHA = 0.3;        // smoothing factor, higher = trusts new readings more
const TREND_DEADBAND = 0.05;   // slope magnitude below this counts as "Stable"

// Risk score weights (PM2.5 is the bigger health hazard at typical
// indoor/outdoor concentrations, so it's weighted higher than CO2)
const RISK_WEIGHT_CO2 = 0.02;
const RISK_WEIGHT_PM25 = 0.3;

// Tier boundaries, evaluated on the smoothed Risk score
const TIER_THRESHOLDS = {
  1: [0, 10],      // Good
  2: [10, 25],     // Moderate
  3: [25, 50],     // Poor  -> triggers a call
  4: [50, Infinity] // Severe -> triggers an urgent call
};
const TIER_LABELS = { 1: "Good", 2: "Moderate", 3: "Poor", 4: "Severe" };

// Which tiers trigger the auto-dial, and the cooldown between repeat calls
// to the same zone (avoids spamming the phone during a sustained event).
const DIAL_ON_TIERS = new Set([3, 4]);
const DIAL_COOLDOWN_MINUTES = 15;

// Zone management contact numbers — replace with real numbers.
// See the "Connecting Twilio" section in README.md for how these get dialed.
const ZONE_CONTACTS = {
...
// Vulnerability weight used only for the priority ranking strip, not Tier
...
class Zone {
  constructor(zoneId, baseCo2, basePm25) {
    ...
    this.spikeUntil = null; // for the manual "trigger spike" demo button
  }
...
let alerts = []; // newest first, capped at 100

// --------------------------------------------------------------------------
// AUTO-DIAL (see README.md "Connecting Twilio" to make this a real call)
// --------------------------------------------------------------------------
...
function dialZoneContact(zone) {
  ...
  if (zone.lastDialTime && (now - zone.lastDialTime) < DIAL_COOLDOWN_MINUTES * 60000) {
    return; // still in cooldown, don't call again yet
  }
...
backgroundTick(); // populate immediately on boot, don't wait 5s for first data
